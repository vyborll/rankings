...
const getUpcomingDropsQuery = gql`
  query {
    upcomingFeatured {
      imageUrl
      name
...
const Upcoming: NextPage = () => {
  const { loading, error, data } = useQuery(getUpcomingDropsQuery);
      ...
      <Layout>
        <div className="space-y-6">
          ...
          <div className="max-w-3xl mx-auto text-center space-y-2">
            <div>
              Some of the information here may not be correct. Please verify with your own research if your are interested in any of the projects
              below.
            </div>
            <div>
        ...
        {!loading && data?.upcoming.length < 1 && data?.upcomingFeatured.length < 1 ? (
        ...
        {!loading &&
          data &&
          data.upcomingFeatured.map((drop: any, i: number) => (
        ...
        {!loading &&
          data &&
          data.upcoming.map((drop: any, i: number) => (

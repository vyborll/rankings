...
const Live: NextPage = () => {
  ...
  const { data, mutate } = useSWR(`/api/mints/top/${timeFrame}`, fetcher, {
    refreshInterval: 20000,
  });

  const changeTimeFrame = async (time: TimeFrame) => {
    if (timeFrame === time) return;

    try {
      setTimeFrame(time);
      mutate(`/api/mints/top/${time}`, false);
    } catch (err) {
      console.error('Error loading top mint collections');
    }
  };
    ...
    <Layout>
      <div className="max-w-5xl mx-auto">
        <div className="space-y-4">
          ...
          <div>Monitor the latest NFT mints on the Ethereum blockchain.</div>
        </div>
            ...
            {['1m', '5m', '10m', '30m', '1h', '6h'].map((time, idx) => (
              <button
                ...
                className={cn('px-4 py-1 rounded text-sm', {
                  'bg-green-950': timeFrame === time,
                  'bg-dark-800': timeFrame !== time,
                })}
                onClick={() => changeTimeFrame(time as TimeFrame)}
              >
                {time}
              </button>
            ))}
...
export const Collection: React.FC<CollectionProps> = ({
  ...
  return (
    <div className="bg-dark-800 p-4 rounded relative hover:shadow-lg z-1">
      <div
        className="absolute top-0 left-0 bottom-0 w-full -z-5 bg-center opacity-3"
        style={{
      ...
      <div className="flex flex-col md:flex-row space-y-4 md:space-y-0">
        <div className="flex flex-1 flex-row items-center space-x-4">
          <img
            className="h-14 w-14 md:h-20 md:w-20 rounded"
            src={imageUrl ?? `https://ui-avatars.com/api/?name=${name}`}
          />
          <div className="flex flex-1 flex-row justify-between md:flex-col md:space-y-1">

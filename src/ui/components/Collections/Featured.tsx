...
const Cards: React.FC<Props> = ({ loading, collections }) => {
	return (
		<>
			<div>
				...
				{!loading &&
					...
					collections.map((collection, i) =>
						i <= 8 ? (
							<motion.div
								key={i}
								className={`bg-dark-800 p-4 rounded relative overflow-hidden${i < 2 ? ' border border-green-950' : ''}`}
								whileHover={{ scale: 1.02 }}
							>
								{i < 2 ? <div className="ribbon bg-green-950 text-base font-bold whitespace-no-wrap px-10 py-1">Featured</div> : null}
								<div className="space-y-4">
									<div className="flex flex-row items-center">
										<div className="relative h-20 w-20">
											<Image src={collection.imageUrl} className="rounded" layout="fill" />
										</div>
										<div className="flex flex-1 flex-col pl-4 space-y-1">
											...
											<div className="bg-dark-900 rounded-full text-center text-sm py-1 font-thin w-40">
												{collection.totalSupply.toLocaleString()} Assets
											</div>
										</div>
									</div>
